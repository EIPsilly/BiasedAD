...
# Controls the number of target categories
@click.option("--nb15_target_class_num" , type=int, default = 3)
...
def main(model_type, dir_path, dataset_name, device, gpu, intermediate_flag, random_seed,
         ...
         s_non_target, s_target, nb15_non_target_class_num, nb15_target_class_num,
    ...
    for i in range(times):
        ...
        if re.search("Unlabelled_data", dataset_name):
            ...
            if model_type == "BiasedAD":
                dataset = NB15Dataset(s_non_target, s_target, nb15_non_target_class_num, nb15_target_class_num, random_seed)
                default_eta_0 = 20
                default_eta_1 = 1
                default_eta_2 = 2
            elif model_type == "BiasedADM":
                dataset = nb15_for_BADM(0, s_target, nb15_non_target_class_num, nb15_target_class_num, random_seed)
                default_eta_0 = 10
                default_eta_1 = 2
                default_eta_2 = 1
            net_name = 'mlp_for_nb15'
            file_name = "s_non_target=" + str(s_non_target) + ",s_target=" + str(s_target) + ",nb15_non_target_class_num=" +str(nb15_non_target_class_num) + "," + f'nb15_target_class_num={nb15_target_class_num},'
            ae_lr = 0.0001 if ae_lr is None else ae_lr
            ae_batch_size = 128 if ae_batch_size is None else ae_batch_size
            ae_epoch = 30 if ae_epoch is None else ae_epoch
            BAD_lr = 0.00001
        ...
        with open(f'result/running_time/{model_type}_{dataset_name}.txt' , "a", encoding="utf-8") as f:
